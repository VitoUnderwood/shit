...
namespace shit {
	...
	float Matrix::dotRow(const Vector &vec, int64_t i) const {
		assert(i >= 0);
		assert(i < m_);
		assert(vec.size() == n_);
		real d = 0.0;
	...
	float Matrix::l2NormRow(int64_t i) const {
		auto norm = 0.0;
		for (auto j = 0; j < n_; j++) {
	...
	void Matrix::save(std::ostream &out) {
		...
		out.write((char *) data_, m_ * n_ * sizeof(float));
	}
	...
	void Matrix::load(std::istream &in) {
		...
		delete[] data_;
		data_ = new real[m_ * n_];
		in.read((char *) data_, m_ * n_ * sizeof(float));
	}
}